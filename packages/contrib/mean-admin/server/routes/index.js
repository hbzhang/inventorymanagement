...
module.exports = function(Admin, app, auth, database) {
    ...
    app.get('/admin/users', auth.requiresLogin, users.all);
    app.get('/supervisor/users', auth.requiresLogin,users.all);
    app.post('/admin/users', auth.requiresAdmin, users.create);
    app.put('/admin/users/:userId', auth.requiresLogin, users.update);
