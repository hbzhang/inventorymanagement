 ...
 */
angular.module('mean.page').controller('SupervisorEditEmployeeSubmissionController', ['$scope','Global',
    'Restangular','Program','$location','$builder','$validator','formtoaster',
    'URLStateTrack','LocalStorageData',
    function($scope,Global,Restangular,Program,$location,
             $builder,$validator,formtoaster,URLStateTrack,
             LocalStorageData) {
        $scope.global = Global;
        ...
        $scope.updateregister = function(){
            return $validator.validate($scope, 'registeredform').success(function() {
                Restangular.one('registration/' + $scope.registrationid).get().then(function(register) {
                    ...
                    register.supervisor_edit = $scope.global.user._id;
                    register.qrcode = [];
                    register.save();
                    $scope.pop();
                });
