 ...
 */
angular.module('mean.page').controller('SupervisorApprovedController', ['$scope','Global',
    'Restangular','Program','$location','$builder','$validator','formtoaster','URLStateTrack',
    'LocalStorageData','Users','UserinformationHelper',
    function($scope,Global,Restangular,Program,$location,
             $builder,$validator,
             formtoaster,
             URLStateTrack,LocalStorageData,Users,UserinformationHelper) {
        $scope.global = Global;
        ...
        $scope.preurl = LocalStorageData.geturlData('supervisorapprove');

        var splitpath = $location.path().split('/')[3];

        $scope.userid = splitpath;

        Users.query({}, function(users) {
            var otherusers = users;
            _.each(users, function(user) {

                if( $scope.userid  === user._id){
        ...
        $scope.getemployeeregisters = function() {
            ...
            });//supervisor form not being used for now
                        ...
                        items.getList().then(function(items) {
                            ...
                            _.each(items,function(value1) {
                                if(value1._id === value.reasonforregister ) {
                                    ...
                                    var forminformation = {};
                                    forminformation.value =  value1.itembasicinformation[0].value + " Comments";
                                    forminformation.label  = "Reviewer ";
